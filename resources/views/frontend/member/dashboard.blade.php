    ...
    <div class="max-w-7xl mx-auto px-4 py-10 space-y-10">
        ...
        @if (session('new_coupon'))
            <div class="fixed top-4 right-4 z-50">
                <div
                    class="bg-gradient-to-r from-blue-400 to-blue-500 text-white shadow-2xl rounded-2xl px-6 py-4 flex items-center space-x-3 animate-fade-in-up">
                    ...
                    Swal.fire({
                        ...
                        if (target) {
                            setTimeout(() => {
                                ...
                                setTimeout(() => target.classList.remove('ring-4', 'ring-blue-400',
                                    'ring-offset-2'), 4000);
                                ...
                                const allSpans = target.querySelectorAll(
                                    'div.flex.justify-between span.font-bold');
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @foreach ($cards as $card)
                <div
                    ...
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-white/70 via-blue-50/60 to-blue-100/40 dark:from-gray-800/70 dark:via-gray-900/50 dark:to-blue-950/30 opacity-90 rounded-2xl">
                    </div>
                    <div class="relative z-10 flex items-center space-x-4">
                        <div
                            class="w-14 h-14 flex items-center justify-center rounded-xl bg-blue-500/10 dark:bg-blue-400/20 text-3xl text-blue-600 dark:text-blue-300 group-hover:scale-110 transition-transform duration-300">
                            <i class="{{ $card['icon'] }}"></i>
                        </div>
                        <div>
                    ...
                    <div
                        class="absolute -bottom-10 right-0 w-32 h-32 bg-gradient-to-tr from-blue-200/20 to-transparent rounded-full blur-2xl">
                    </div>
                </div>
            @endforeach
        </div>
        ...
        {{-- 🎟️ KUPON AKTIF --}}
        <div
            class="bg-gradient-to-br from-white via-blue-50 to-blue-100 dark:from-gray-800 dark:via-gray-900 dark:to-blue-950 backdrop-blur-xl rounded-2xl shadow-lg border border-blue-100/50 dark:border-gray-700 p-6 transition">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">🎟️ Kupon Aktif Anda
            </h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($coupons as $coupon)
                        <div
                            ...
                            <div
                                class="absolute top-0 bottom-0 left-0 w-6 bg-gradient-to-b from-transparent via-white/40 to-transparent dark:via-gray-600/40 rounded-r-full">
                            </div>
                            <div
                                class="absolute top-0 bottom-0 right-0 w-6 bg-gradient-to-b from-transparent via-white/40 to-transparent dark:via-gray-600/40 rounded-l-full">
                            </div>
                            <div
                                class="absolute inset-y-0 left-1/2 transform -translate-x-1/2 w-[2px] border-dashed border-blue-300/50 dark:border-blue-500/30 border-l">
                            </div>

                            <div class="relative z-10 grid grid-cols-2 items-center">
                                <div class="space-y-2">
                                    <p
                                        class="text-xs uppercase tracking-widest font-semibold text-gray-500 dark:text-gray-400">
                                        Kode Kupon</p>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        Kadaluarsa:
                                        {{ $coupon->expiry_date ? \Carbon\Carbon::parse($coupon->expiry_date)->format('d M Y') : 'Tidak ada' }}
                                    </p>
                                </div>

                                <div class="flex flex-col items-center justify-center space-y-2">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold border border-blue-300/40 dark:border-gray-600/60
                                    ...
                                    <button
                                        class="mt-2 bg-gradient-to-r from-blue-500/10 to-blue-300/20 hover:from-blue-500/20 hover:to-blue-400/30 text-gray-900 dark:text-white text-xs font-medium rounded-lg px-3 py-1 transition-all duration-200">
                                        Gunakan Sekarang
                                    </button>
                                </div>
                            </div>
        ...
        <div class="bg-gradient-to-br from-white via-blue-50 to-blue-100 dark:from-gray-900 dark:via-gray-950 dark:to-blue-950 rounded-3xl shadow-2xl border border-blue-100/50 dark:border-gray-800/70 p-6 md:p-8 backdrop-blur-lg overflow-hidden"
            x-data="{ open: true }">
            <div class="flex justify-between items-center mb-8">
                ...
                <button @click="open = !open"
                    class="text-sm text-blue-600 dark:text-blue-400 font-medium hover:underline transition">
            ...
            <div x-show="open" x-transition>
                @if ($transactions->isEmpty())
                    <div
                        class="p-8 bg-white/60 dark:bg-gray-800/50 text-gray-700 dark:text-gray-300 rounded-2xl text-center border border-blue-100/50 dark:border-gray-700 shadow-inner">
                ...
                @else
                    <div class="grid gap-6 sm:gap-8 md:grid-cols-2 xl:grid-cols-3">
                        @foreach ($transactions as $trx)
                            ...
                            <div
                                id="trx-{{ $trx->transaction_code }}"class="group relative bg-white/80 dark:bg-gray-800/70 backdrop-blur-md rounded-2xl border border-blue-100/70 dark:border-gray-700 p-6 shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 overflow-hidden">
                                <div
                                    class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 via-indigo-400 to-purple-500 rounded-t-2xl">
                                </div>

                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3
                                            class="text-lg font-semibold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
                                ...
                                <div class="space-y-3 text-sm text-gray-600 dark:text-gray-300">
                                    ...
                                    @if ($trx->payment_status === 'DP' && $remaining > 0)
                                        <div
                                            class="mt-4 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 flex justify-between items-center">
                                ...
                                <div
                                    class="mt-6 pt-4 border-t border-blue-100/50 dark:border-gray-700 flex justify-between items-center flex-wrap gap-3">
                                    ...
                                    @php
                                        $appointment = $trx->appointment;
                                        $canReschedule =
                                            ($appointment &&
                                                $appointment->booking_date &&
                                                \Carbon\Carbon::parse($appointment->booking_date)->isToday()) ||
                                            \Carbon\Carbon::parse($appointment->booking_date)->isFuture();

                                        $blockedStatus = ['Cancelled', 'Completed'];
                                    @endphp

                                    @if ($canReschedule && !in_array($appointment->status, $blockedStatus))
                                        <button type="button"
                                            class="btn-reschedule inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium
               bg-indigo-500/10 text-indigo-600 hover:bg-indigo-500/20 transition-all"
                                            data-appointment-id="{{ $appointment->id }}"
                                            data-employee-id="{{ $appointment->employee_id }}"
                                            data-service-id="{{ $appointment->service_id }}"
                                            data-slot-group-id="{{ $appointment->slot_group_id }}">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content rounded-2xl">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" id="reschedule-submit-btn">
                        Simpan Perubahan
                    </button>
                </div>
            </div>
    ...
    <script>
        document.querySelectorAll('.btn-reschedule').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                container.innerHTML = '<span class="text-muted">Pilih tanggal terlebih dahulu</span>';
            ...
            fetch(`/appointments/${appointmentId}/reschedule/availability?date=${date}`)
                ...
                .then(res => {
                    ...
                    res.available_slots.forEach(slot => {
                        ...
                        btn.onclick = () => {
                            document
                                .querySelectorAll('.btn-reschedule-slot')
                ...
                .catch(() => {
                    container.innerHTML =
                        '<div class="text-danger">Gagal memuat slot</div>';
                });
        }
    </script>

    <script>
        document.getElementById('reschedule-submit-btn')?.addEventListener('click', () => {
            ...
            const slot =
                document.querySelector('.btn-reschedule-slot.active');
