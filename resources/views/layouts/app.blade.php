...
<body class="min-h-screen bg-gray-100 dark:bg-gray-900 font-sans"
      x-data="navbar()" x-init="init()" x-cloak
...
<div 
    ...
    <div class="relative inline-block">
        <!-- Logo dark -->
        <img src="{{ asset('uploads/images/logo-no-o-hitam.webp') }}" 
             alt="Logo hitam" 
             class="h-60 w-auto block dark:hidden">
        <!-- Logo light -->
        <img src="{{ asset('uploads/images/logo-no-o-putih.webp') }}" 
             alt="Logo putih" 
             class="h-60 w-auto hidden dark:block">

        <!-- Huruf O pertama -->
        <img src="{{ asset('uploads/images/logo-o-hitam.webp') }}" 
             alt="O pertama hitam" 
             class="absolute animate-spin-slow object-contain dark:hidden"
             style="top: 67px; left: 45px; height: 97.5px; width: 97.5px;">
        <img src="{{ asset('uploads/images/logo-o-putih.webp') }}" 
             alt="O pertama putih" 
    ...
    <!-- App Wrapper -->
    <div id="app" 
         :class="{'opacity-0': loading, 'opacity-100 transition-opacity duration-500': !loading}" 
...
<nav 
    ...
    <div class="flex h-16 items-center justify-between relative">
        ...
        <a href="/" class="flex-shrink-0 h-16 w-16 block">
            <img src="{{ asset('uploads/images/logohitam.webp') }}" class="h-full w-full object-contain block dark:hidden">
            <img src="{{ asset('uploads/images/logoputih.webp') }}" class="h-full w-full object-contain hidden dark:block">
        </a>
...
<div class="hidden lg:flex absolute left-1/2 transform -translate-x-1/2 
    ...
    <a href="/" class="rounded-xl px-4 py-2 text-sm font-medium transition 
    ...
    <a href="{{ route('booking') }}" class="rounded-xl px-4 py-2 text-sm font-medium transition 
    ...
    <a href="/pricelist" class="rounded-xl px-4 py-2 text-sm font-medium transition 
    ...
    <a href="/gallery" class="rounded-xl px-4 py-2 text-sm font-medium transition 
    ...
    <a href="/claim-photos" class="rounded-xl px-4 py-2 text-sm font-medium transition 
...
<div class="nav-right hidden lg:flex items-center space-x-4">
    ...
    <button @click="toggleDarkMode" 
        ...
        <template x-if="!darkMode"><i class="fa fa-moon w-4 h-4 text-center leading-none"></i></template>
        <template x-if="darkMode"><i class="fa fa-sun w-4 h-4 text-center leading-none"></i></template>
    </button>
...
<div class="lg:hidden flex items-center space-x-2">
    ...
    <button @click="mobileMenu = !mobileMenu" 
        ...
        <template x-if="!mobileMenu"><i class="fa fa-bars w-4 h-4 text-center leading-none text-lg"></i></template>
        <template x-if="mobileMenu"><i class="fa fa-times w-4 h-4 text-center leading-none text-lg"></i></template>
    </button>
...
>
    ...
    <div x-show="mobileMenu"
        ...
        <div class="space-y-1 mt-2">
            <!-- User Info -->
            <div class="flex items-center px-3 py-2 rounded-md bg-gradient-to-r from-blue-50/40 to-blue-100/30 dark:from-gray-800/60 dark:to-gray-900/40 shadow-inner">
                <img src="{{ Auth::user()->profile_picture ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name) }}" 
                     class="h-10 w-10 rounded-full mr-3 border border-blue-200 dark:border-gray-600 shadow-sm">
                <div class="flex flex-col text-left">
                    <span class="text-sm font-semibold text-blue-900 dark:text-gray-100 truncate">{{ Auth::user()->name }}</span>
                    <span class="text-[8px] text-blue-500 dark:text-gray-500 mt-0.5">{{ Auth::user()->role_uid ?? 'USER' }}</span>
                </div>
            </div>
